// =============================================
// script.js — دم الخير — النسخة المصححة
// =============================================
...
const ADMIN_EMAILS  = ['sales.burjuman1@gmail.com'];
 
if (typeof auth !== 'undefined') {
    auth.onAuthStateChanged(async user => {
        const page = window.location.pathname.split('/').pop() || 'index.html';
 
        // admin.html تدار بكودها الخاص
...
async function handleRegister(e) {
    ...
    // معلومات المتبرع الصحية الإضافية
    const lastDonationVal      = document.getElementById('lastDonationDate')?.value || null;
    ...
    const chronicDiseaseDetails = document.getElementById('chronicDiseaseDetails')?.value?.trim() || '';
 
    // تحقق من فترة 3 أشهر — فقط للمتبرعين
    if (role === 'donor' && lastDonationVal) {
    ...
    try {
        ...
        const userData = {
            ...
            age:       parseInt(age) || 0,
            bloodType, role,
            donateType:      document.getElementById('donateType')?.value || 'blood',
            available:       true,
            totalDonations:  0,
            lastDonation:    lastDonationVal || null,
            isSmoker,
            hasChronicDisease,
...
async function handleRequest(e) {
    ...
    try {
        await db.collection('blood_requests').add({
            ...
            patientChronic:      document.getElementById('patientChronic')?.value || 'no',
            patientBloodDisease: document.getElementById('patientBloodDisease')?.value || 'no',
            patientBloodDiseaseDetails: document.getElementById('patientBloodDiseaseDetails')?.value?.trim() || '',
            city, hospital, phone,
...
function loadRequests(containerId, limitCount=20, urgentOnly=false) {
    ...
    q.onSnapshot(snap => {
        ...
        // إذا Index مفقود — جرب بدون orderBy
        if (err.code === 'failed-precondition' || err.message?.includes('index')) {
            db.collection('blood_requests').where('status','==','active').limit(limitCount)
            .onSnapshot(snap2 => {
                _allRequests = snap2.docs.map(doc => ({ id: doc.id, ...doc.data() }));
                const countEl = document.getElementById('requestsCount');
                if (countEl) countEl.textContent = _allRequests.length;
                renderFilteredRequests();
            });
...
function filterRequests() {
    ...
    if (container) container.innerHTML = filtered.length
        ...
        : '<div style="text-align:center;padding:60px;color:var(--gray-400)"><i class="fas fa-search fa-2x" style="margin-bottom:12px;display:block"></i><p>لا توجد نتائج بهذه المعايير</p></div>';
}
...
function renderDonorCard(donor) {
    ...
    <div class="donor-card fade-in">
        ...
        <div class="donor-meta">
            ...
            ${donor.donateType === 'platelets' ? '<span style="background:#F5F3FF;color:#7C3AED;font-size:0.7rem;font-weight:700;padding:2px 8px;border-radius:50px;border:1px solid rgba(124,58,237,0.2)"><i class=\"fas fa-microscope\"></i> صفائح دموية</span>' : ''}
...
function selectType(type) {
    ...
    // إظهار/إخفاء حقول المتبرع الصحية حسب نوع الحساب
    const healthFields = document.getElementById('donorHealthFields');
    if (healthFields) healthFields.style.display = type === 'donor' ? 'block' : 'none';
}
...
// ===== اختيار نوع التبرع (دم كامل / صفائح) =====
function selectDonateType(type) {
    document.getElementById('donateType').value = type;
    const bloodBtn     = document.getElementById('donateTypeBlood');
    const plateletsBtn = document.getElementById('donateTypePlatelets');
    if (!bloodBtn || !plateletsBtn) return;
    if (type === 'blood') {
        bloodBtn.style.cssText     = 'padding:14px 10px;border:2px solid var(--red);background:var(--red-faint);border-radius:12px;cursor:pointer;font-family:var(--font-main);transition:all 0.2s;text-align:center';
        plateletsBtn.style.cssText = 'padding:14px 10px;border:2px solid var(--gray-200);background:white;border-radius:12px;cursor:pointer;font-family:var(--font-main);transition:all 0.2s;text-align:center';
        bloodBtn.querySelector('span').style.color     = 'var(--red)';
        plateletsBtn.querySelector('span').style.color = '#6B6B6B';
    } else {
        plateletsBtn.style.cssText = 'padding:14px 10px;border:2px solid #7C3AED;background:#F5F3FF;border-radius:12px;cursor:pointer;font-family:var(--font-main);transition:all 0.2s;text-align:center';
        bloodBtn.style.cssText     = 'padding:14px 10px;border:2px solid var(--gray-200);background:white;border-radius:12px;cursor:pointer;font-family:var(--font-main);transition:all 0.2s;text-align:center';
        plateletsBtn.querySelector('span').style.color = '#7C3AED';
        bloodBtn.querySelector('span').style.color     = '#6B6B6B';
    }
}
